...
/* Uncomment the line below if you want to use user defined Delay function
   ...
   the Ethernet driver is used (less precise timing) 
	 如果要使用用户定义的延迟功能（用于精确计时），请取消注释下面的行，
	 否则，将使用以太网驱动程序中定义的默认_eth_delay_函数（精确度较低的计时）*/
//#define USE_Delay
  ...
  #define _eth_delay_    Delay     /* User can provide more timing precise _eth_delay_ function
                                      ...
                                      STM32F4x7xx demonstrations) 
																			用户可以提供更多定时精确的_eth_delay_函数。
																			使用时基为10 ms的Systick（如提供的STM32F4x7xx演示中所述）*/
#else
	extern void ETH_Delay(__IO uint32_t nCount);
  #define _eth_delay_    ETH_Delay /* Default _eth_delay_ function with less precise timing默认的_eth_delay_函数，时序不太精确 */
#endif
