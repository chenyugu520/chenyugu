...
MQTT_SEND_START:
    ...
    xReturn = xQueueReceive( MQTT_Data_Queue,    /* 消息队列的句柄 */
      ...
      if(xReturn == pdTRUE)
      {
       // a = ((double)(recv_data->humi_int * 100 + recv_data->humi_deci))/100;//湿度
       // b = ((double)(recv_data->temp_int * 100 + recv_data->temp_deci))/100;//温度
//        printf("a = %f,b = %f\n",a,b);
        //更新数据      
...
{
  sys_thread_new("mqtt_thread", mqtt_thread, NULL,1024, 6);
