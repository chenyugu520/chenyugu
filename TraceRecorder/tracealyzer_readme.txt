...
Tracealyzer是用于跟踪和可视化基于FreeRTOS的软件系统的复杂工具。 Tracealyzer对运行时行为提供了前所未有的洞察力，从而加快了调试，验证和优化的速度。跟踪记录器库（Trace Recorder Library）是Tracealyzer的目标端部分，它执行实际的跟踪。然后可以在Tracealyzer PC应用程序中查看生成的数据
